...
bool checkStep(char visibleTable[][EDGE], char hiddenTable[][EDGE],
    int mines[][2], int row, int column, int* movesLeft)
{

    if (visibleTable[row][column] != '-')
        return (true);

    if (hiddenTable[row][column] == '*')
    {
        for (int i = 0; i < MINES; i++)
            visibleTable[mines[i][0]][mines[i][1]] = '*';
        ...
        if (!amount)
        {
            ...
            if (isInside(row - 1, column) == true)
            {
                if (isMine(row - 1, column, hiddenTable) == false)
                    checkStep(visibleTable, hiddenTable, mines, row - 1, column, movesLeft);
            }
            ...
            if (isInside(row, column + 1) == true)
            {
                if (isMine(row, column + 1, hiddenTable) == false)
                    checkStep(visibleTable, hiddenTable, mines, row, column + 1, movesLeft);
            }
            ...
            if (isInside(row - 1, column + 1) == true)
            {
                if (isMine(row - 1, column + 1, hiddenTable) == false)
                    checkStep(visibleTable, hiddenTable, mines, row - 1, column + 1, movesLeft);
            }
...
void placeMines(int mines[][2], char hiddenTable[][EDGE])
{
    ...
    for (int i = 0; i < MINES;)
    {
        ...
        if (marked[randomNumber] == false)
        {
            ...
            hiddenTable[mines[i][0]][mines[i][1]] = '*';
...
void playMinesweeper()
{
    ...
    int movesLeft = EDGE * EDGE - MINES, r, c;
    int mines[MINES][2]; 
    ...
    while (gameInPlay == true)
    {
        ...
        gameInPlay = checkStep(visibleTable, hiddenTable, mines, r, c, &movesLeft);

        if ((gameInPlay == true) && (movesLeft == 0))
        {
            cout << "\nYou won !" << endl;
            gameInPlay = false;
        }
